...
namespace Exercice1._1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            var peachProducers = (from p in products where p.ProductName == "Pêches" select p.Producer).Count();
            Console.WriteLine($"There are {peachProducers} producers selling peaches.");


            var producerWithMostMelons = (from p in products where p.ProductName == "Pastèques" orderby p.Quantity select p.Producer).First();
            Console.WriteLine($"{producerWithMostMelons} is the producer who sells the most watermelons.");


            var producerWithMostProducts = (from p in products group p by p.Producer);
            Console.WriteLine($"{producerWithMostProducts}");
        }
    }
}
